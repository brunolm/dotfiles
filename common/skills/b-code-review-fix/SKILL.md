...
Then ask the user via `AskUserQuestion` and fold the proposed fix(es) into the options. Each fix option's **label** is `Fix — <short fix name>` (3-6 words naming the approach) and its **description** is a one-sentence summary of the concrete change (which function, which line, which API).

If there's a single sensible fix, present one Fix option. If there are multiple viable approaches (e.g., narrow patch vs. proper refactor, or two different APIs that resolve the issue), present each as its own Fix option — keep it to a maximum of two so the prompt stays inside the 4-option limit.
