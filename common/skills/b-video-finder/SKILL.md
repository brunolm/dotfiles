...
- **≤ 30 seconds per video**, total wall-clock from navigating to it until the probe returns. The budget wins over extra frames. If a video won't load, won't seek, or hits DRM/region walls inside the budget, score whatever files exist (or `C` if none) and move on — never burn the budget retrying.
