---
name: brunolm-toggl-overtime
description: Use this skill when the user wants to know their Toggl overtime for a month. Triggers include "/brunolm-toggl-overtime", "how much overtime do I have", "toggl overtime for June", "am I ahead or behind on hours this month", or any phrasing pairing Toggl with overtime / hour balance. Assumes 8h of work per workday (Mon–Fri). For the current month it counts from the 1st through yesterday — today is excluded unless the user explicitly asks to include it; for a past month it counts the whole month. Pulls tracked time from the Toggl MCP, compares it against expected hours, and reports the overtime (or deficit) with a per-day breakdown on request. Accepts an optional month argument like "june", "2026-06", or "last month"; default is the current month.
version: 1.0.0
model: haiku
effort: low
allowed-tools:
...
# Toggl overtime (8h/workday baseline)

Compare the time tracked in Toggl against an expected **8 hours per workday (Mon–Fri)** and report the overtime or deficit. This skill only **reads** from Toggl — it never creates, updates, or deletes anything.
...
## 3. Pull the tracked time from Toggl

1. Call `me` to get the default workspace id (fall back to `list_workspaces` if needed).
2. Call `summary_report` for the workspace with `start_date` / `end_date` from step 2 to get the total tracked duration for the period. If the report only returns per-project/per-day buckets, sum them.
3. If the period includes today, call `get_current_time_entry` — a **running** timer is usually not in the report yet. If one is running and it started within the period, add its elapsed time (now − start) to the total. Mention that a running timer was included.

If `summary_report` can't produce a plain total, fall back to `list_time_entries` for the range and sum the durations (Toggl durations are in **seconds**; a running entry has a negative duration — skip it there, it's already covered by step 3.3).

## 4. Compute and report the overtime

Compute in PowerShell (feed in the tracked total in seconds):

```powershell
$tracked = [timespan]::FromSeconds($trackedSeconds)
...
- **Workdays** — count, and the expected hours (`workdays × 8h`).
...
Only produce a per-day breakdown (a table of date, tracked, ±diff vs 8h, weekends marked) if the user asks for it — that needs `list_time_entries` or a day-grouped `summary_report`.
