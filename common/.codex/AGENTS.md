...
- Web fetching fallback chain: try the built-in web fetch first; if it fails or is blocked (403/406, bot detection, challenge page, empty content), use the `stealth-fetch` MCP `fetch` tool; if that also fails, drive a real browser via the `patchright` MCP.
