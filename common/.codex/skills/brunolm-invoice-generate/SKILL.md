...
**If no source was provided, ask** which to use: get from Toggl, get from Harvest, or enter a number. Fixed-quantity items (e.g. a product with quantity 1) are unaffected.

## 4. Compute the amounts

For each item: `Amount = Quantity x Unit Price`. The **amount due** is the sum of all item amounts. Compute in PowerShell (or with obvious arithmetic double-checked), format with two decimals and thousands separators, and prefix with the currency.
...
AskUserQuestion is a Claude Code tool; when running under Codex, just ask the questions in plain conversation instead.
