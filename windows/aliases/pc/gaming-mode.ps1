...
function B-PC-Set-GamingMode {
  ...
  if ($stoppable) {
    $note = if (B-PC-IsElevated) { '' } else { ' - will be SKIPPED, needs elevation' }
  ...
  Write-Host "Undo with: B-PC-Set-GamingMode off" -ForegroundColor DarkGray
}
...
function PCGaming-StopServices($names) {
  ...
  if (!(B-PC-IsElevated)) {
    Write-Host "Skipped $($names.Count) service(s) - needs an elevated shell (B-PC-Start-Powershell -Elevated):" -ForegroundColor Yellow
...
function PCGaming-Restore {
  ...
  if ($services) {
    if (!(B-PC-IsElevated)) {
      Write-Host "Cannot restart services - needs an elevated shell (B-PC-Start-Powershell -Elevated):" -ForegroundColor Yellow
